...
function confirm_due_at(string $bizDate, string $planTime, string $businessDayStart, int $leadHours): ?DateTime {
  ...
  if ($leadHours > 0) {
    $base->modify('-' . $leadHours . ' hours');
  }
  return $base;
}

function quick_reply_items(): array {
  $items = [];
  foreach ([
    ['label' => '出勤します', 'text' => '出勤します'],
    ['label' => '遅れます', 'text' => '遅れます'],
    ['label' => '休みます', 'text' => '休みます'],
    ['label' => 'その他', 'text' => 'その他'],
  ] as $item) {
    $items[] = [
      'type' => 'action',
...
function parse_cli_options(array $argv): array {
  ...
  foreach (array_slice($argv, 1) as $arg) {
    ...
    if (strpos($arg, '--lead-hours=') === 0) {
      $value = substr($arg, strlen('--lead-hours='));
      if ($value !== '' && ctype_digit($value)) {
        $opts['lead_hours'] = max(0, (int)$value);
      }
    }
  }
...
$leadHours = (int)$opts['lead_hours'];
$accessToken = conf('LINE_MSG_CHANNEL_ACCESS_TOKEN');

if (!$dryRun && $accessToken === '') {
  fwrite(STDERR, "[attendance_confirm] LINE_MSG_CHANNEL_ACCESS_TOKEN missing\n");
  exit(1);
}
...
$hasEnabledColumn = table_has_column($pdo, 'stores', 'attendance_confirm_auto_enabled');
$hasLeadColumn = table_has_column($pdo, 'stores', 'attendance_confirm_lead_hours');
$enabledCol = $hasEnabledColumn ? 'attendance_confirm_auto_enabled' : '1 AS attendance_confirm_auto_enabled';
$leadCol = $hasLeadColumn ? 'attendance_confirm_lead_hours' : (string)$leadHours . ' AS attendance_confirm_lead_hours';

$stores = $pdo->query("
  SELECT id, name, business_day_start, {$enabledCol}, {$leadCol}
  FROM stores
  WHERE is_active = 1
  ORDER BY id ASC
...
foreach ($stores as $store) {
  ...
  $autoEnabled = ((int)($store['attendance_confirm_auto_enabled'] ?? 1) === 1);
  if (!$autoEnabled) {
    $skipCount++;
    fwrite(STDOUT, sprintf(
      "[attendance_confirm] skip store=%d name=%s reason=auto_disabled\n",
      $storeId,
      $storeName
    ));
    continue;
  ...
  if ($senderUserId <= 0) {
    $skipCount++;
    fwrite(STDOUT, sprintf(
      "[attendance_confirm] skip store=%d name=%s reason=no_sender_user\n",
      $storeId,
      $storeName
    ));
    continue;
  ...
  $storeLeadHours = (int)($store['attendance_confirm_lead_hours'] ?? $leadHours);
  if ($storeLeadHours < 0) $storeLeadHours = 0;
  if ($storeLeadHours > 24) $storeLeadHours = 24;
  ...
  foreach ($candidates as $row) {
    ...
    if (!empty($row['clock_in'])) {
      $skipCount++;
      fwrite(STDOUT, sprintf(
        "[attendance_confirm] skip store=%d cast=%d name=%s reason=already_clocked_in\n",
        $storeId,
        $castUserId,
        $displayName
      ));
    ...
    if ($sentCheck->fetchColumn()) {
      $skipCount++;
      fwrite(STDOUT, sprintf(
        "[attendance_confirm] skip store=%d cast=%d name=%s reason=already_sent\n",
        $storeId,
        $castUserId,
        $displayName
      ));
    ...
    if ($lineUserId === '') {
      $skipCount++;
      fwrite(STDOUT, sprintf(
        "[attendance_confirm] skip store=%d cast=%d name=%s reason=line_unlinked\n",
        $storeId,
        $castUserId,
        $displayName
      ));
    ...
    if (!$dueAt || $dueAt > $now) {
      $skipCount++;
      fwrite(STDOUT, sprintf(
        "[attendance_confirm] skip store=%d cast=%d name=%s reason=not_due due_at=%s now=%s\n",
        $storeId,
        $castUserId,
        $displayName,
    ...
    $planStart = substr((string)($row['start_time'] ?? ''), 0, 5);
    $message = $displayName . "さん\n本日 {$planStart} 出勤予定です。\n出勤可否を選んで返信してください。";
    $token = bin2hex(random_bytes(12));

    if ($dryRun) {
      fwrite(STDOUT, sprintf(
        "[attendance_confirm dry-run] store=%d cast=%d name=%s due_at=%s\n",
        $storeId,
        $castUserId,
        $displayName,
    ...
    try {
      ...
      if ($api['code'] >= 300) {
        ...
        fwrite(STDERR, sprintf(
          "[attendance_confirm] send failed store=%d cast=%d msg=%s\n",
          $storeId,
          $castUserId,
          $errMsg
        ));
      ...
      $sentCount++;
      fwrite(STDOUT, sprintf(
        "[attendance_confirm] sent store=%d cast=%d name=%s\n",
        $storeId,
        $castUserId,
        $displayName
      ));
      ...
      $failCount++;
      fwrite(STDERR, sprintf(
        "[attendance_confirm] exception store=%d cast=%d err=%s\n",
        $storeId,
        $castUserId,
        $e->getMessage()
      ));
...
fwrite(STDOUT, sprintf(
  "[attendance_confirm] done sent=%d skipped=%d failed=%d mode=%s lead=%dh\n",
  $sentCount,
  $skipCount,
  $failCount,
  $dryRun ? 'dry-run' : 'live',
